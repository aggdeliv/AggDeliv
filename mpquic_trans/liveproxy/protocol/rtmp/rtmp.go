...
import (
	...
	"mp_quic/liveproxy/container/flv"
	"mp_quic/liveproxy/protocol/rtmp/core"
	"mp_quic/liveproxy/utils/uid"
	"net"
	"net/url"
	"reflect"
	"strings"
...
var (
	...
	// revision start
	rtmpFrame = make(chan core.ChunkStream, 10)
	role      int
	// revision end
)

// revision start
func GetRTMPFrame(chunk core.ChunkStream) core.ChunkStream {
	chunk = <-rtmpFrame
	return chunk
}

func SetRTMPFrame(chunk core.ChunkStream) {
	rtmpFrame <- chunk
}

func SetRole(roleInd int) {
	role = roleInd
}
...
func (s *Server) handleConn(conn *core.Conn) error {
	...
	if connServer.IsPublisher() {
		// revision start
		if role != SERVER_IND { // revision end
			...
			//channel, _ := configure.RoomKeys.GetChannel(name)
...
func NewVirWriter(conn StreamReadWriteCloser) *VirWriter {
	ret := &VirWriter{
		Uid:         uid.NewId(),
		conn:        conn,
		RWBaser:     av.NewRWBaser(time.Second * time.Duration(writeTimeout) * 10), // revision start & end
...
func (v *VirReader) Read(p *av.Packet) (err error) {
	...
	for {
		...
		if role == SERVER_IND {
			cs = <-rtmpFrame
			if int(cs.Length) != len(cs.Data) {
				log.Infof("quic packet size=%d", cs.Length)
				log.Infof("rtmp packet size=%d", len(cs.Data))
			}
