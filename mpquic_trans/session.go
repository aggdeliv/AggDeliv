...
import (
	...
	"mp_quic/ackhandler"
	"mp_quic/congestion"
	"mp_quic/internal/flowcontrol"
	"mp_quic/internal/handshake"
...
const (
	bwFreq         = time.Millisecond * 100
	dcFreq         = time.Millisecond * 1000
	pktSize  int   = 1350
	cid_dc   int64 = 1
	cid_bw   int64 = 2
	grpc_url       = "localhost:23333"
)
...
type session struct {
	...
	// revision start
	//bandwidth   map[protocol.PathID]uint64
	probability map[protocol.PathID]float64
	//sendTime    map[protocol.PacketNumber]int64
	pktLen     map[protocol.PacketNumber]int
	metaLock   sync.RWMutex
	bwLock     sync.RWMutex
	probLock   sync.RWMutex
...
// revision start
// calculate the bandwidth of each path (to be revised!!)
func (s *session) BandwidthEstimate() map[protocol.PathID]uint64 {
	var bandwidth map[protocol.PathID]uint64
	bandwidth = make(map[protocol.PathID]uint64)
	//s.bwLock.Lock()
	for key, val := range s.pathManager.oliaSenders {
		//s.bandwidth[key] = uint64(val.BandwidthEstimate())
		//bandwidth[key] = uint64(val.BandwidthEstimate())
		bandwidth[key] = uint64(val.GetCongestionWindow())
		//if val.InSlowStart() {
		//	bandwidth[key] = uint64(val.BandwidthEstimate())
		//} else {
		//	bandwidth[key] = uint64(val.GetCongestionWindow())
		//}
		//bandwidth[key] = uint64(val.TmpBwt)
		//println("pathID:", key)
		//val.BandwidthEstimate()
		//log.Infof("pathID= %v, TmpBwt=%v, Estimate= %v", key, uint64(val.TmpBwt), uint64(val.BandwidthEstimate()))
		//val.PrintCongestionWindow()
	}
	//s.bwLock.Unlock()
	return bandwidth
}

func (s *session) InteractWithModel() {
	conn, err := grpc.Dial(grpc_url, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		log.Errorf("panic_grpc: %v", err)
		return
	}
	stub := pb.NewGRPCDemoClient(conn)

	go func() {
		defer conn.Close()
		//rand.Seed(time.Now().Unix())
		for {
			//if s.platform == uint8(0) {
			//	s.platform = <-s.platChan
			//	if s.platform != uint8(1) {
			//		break
			//	}
			//}
			bwInfo := "B\t"
			...
			s.pathIDLock.Lock()
			s.pathIDArr = []int{}
			for id, _ := range bandwidth {
				s.pathIDArr = append(s.pathIDArr, int(id))
			}
			sort.Ints(s.pathIDArr)
			for _, key := range s.pathIDArr {
				if bw, ok := bandwidth[protocol.PathID(key)]; ok {
					//if _, ok := bandwidth[protocol.PathID(key)]; ok {
					//bw = uint64(10 + rand.Int63n(100-10))
					bwInfo += strconv.Itoa(int(bw))
					//bwInfo += strconv.Itoa(330159768)
					if count < len(bandwidth)-1 {
						bwInfo += "\t"
					}
				}
				count++
			}
			//println("bwInfo:", bwInfo)
			s.pathIDLock.Unlock()
			//s.bwLock.RUnlock()

			if len(bandwidth) != 0 {
				request := pb.Request{ClientId: cid_bw, RequestData: bwInfo}
				ctx, cancel := context.WithTimeout(context.Background(), time.Second)
				defer cancel()

				response, err := stub.SimpleMethod(ctx, &request)
				if err != nil {
					//log.Errorf("panic_grpc_bw: %v.SimpleMethod(_) = _, %v", stub, err)
				} else {
					//smp_num, _ := strconv.Atoi(response.ResponseData)
					strconv.Atoi(response.ResponseData)
					//log.Infof("bw sample num: %v", smp_num)
				}
			}
			time.Sleep(bwFreq)
		}
	...
	go func() {
		...
		for {
			...
			//	}
			//}
			//chunkSize := <-s.sizeChan
			//transTime := int(<-s.timeChan)
			//if chunkSize == 0 {
			//	continue
			//}
			//if transTime == 0 {
			...
			if now-s.startTime == 0 {
				continue
			}
			//s.sizeLock.RLock()
			//curSentSize := s.sentSize
			aggThp := s.sentSize * int(time.Second) / int(now-s.startTime) * 8
			//println("deltaSize:", curSentSize-lastSentSize)
			//lastSentSize = curSentSize
			//s.sizeLock.RUnlock()
			//s.metaLock.Unlock()
			inputInfo := "D\t"
			inputInfo += strconv.Itoa(aggThp) + "\t"
			//inputInfo += strconv.Itoa(chunkSize) + "\t"
			//inputInfo += strconv.Itoa(transTime) + "\t"

			request := pb.Request{ClientId: cid_dc, RequestData: inputInfo}
			ctx, cancel := context.WithTimeout(context.Background(), time.Second)
			defer cancel()
			...
			if err != nil {
				//s.probLock.Lock()
				//total := 0.0
				//for i := 0; i < len(s.pathIDArr); i++ {
				//	idx := protocol.PathID(s.pathIDArr[i])
				//	tmp := rand.Float64()
				//	s.probability[idx] = tmp
				//	total += tmp
				//}
				//for i := 0; i < len(s.pathIDArr); i++ {
				//	idx := protocol.PathID(s.pathIDArr[i])
				//	tmp := s.probability[idx]
				//	s.probability[idx] = tmp / total
				//}
				//s.probLock.Unlock()
				//log.Errorf("panic_grpc_dc: %v.SimpleMethod(_) = _, %v", stub, err)
			} else {
				splitRes := strings.Split(response.ResponseData, "&")
				s.pathIDLock.RLock()
				if len(splitRes) != len(s.pathIDArr) {
					//println("nSplitRes:", len(splitRes), ", nPathIDArr:", len(s.pathIDArr))
					log.Errorf("panic_grpc: inconsistent path number")
				} else {
					prob_info := ""
					s.probLock.Lock()
					for i := 0; i < len(s.pathIDArr); i++ {
						s.probability[protocol.PathID(s.pathIDArr[i])], _ = strconv.ParseFloat(splitRes[i], 64)
						prob_info += splitRes[i] + "\t"
					}
					//println("----------")
					...
					//}
					s.probLock.Unlock()
					//log.Info("path prob: " + prob_info)
				}
				s.pathIDLock.RUnlock()
			}
			time.Sleep(dcFreq)
		}
	}()
...
func (s *session) setup(
	...
	// revision start
	//s.bandwidth = make(map[protocol.PathID]uint64)
	s.probability = make(map[protocol.PathID]float64)
	s.startTime = 0
	s.sentSize = 0
	//s.sendTime = make(map[protocol.PacketNumber]int64)
	s.pktLen = make(map[protocol.PacketNumber]int)
	s.sizeChan = make(chan int, 10000)
	s.timeChan = make(chan int64, 10000)
	s.pathIDArr = []int{}
	//s.platform = uint8(0)
	s.InteractWithModel()
	// revision end
...
func (s *session) handleAckFrame(frame *wire.AckFrame) error {
	//println("handleAckFrame(...)")
	//ackRecvTime := time.Now().UnixNano() / int64(time.Millisecond)
	...
	if _, ok := s.pathManager.oliaSenders[pth.pathID]; ok {
		s.pathManager.oliaSenders[pth.pathID].UpdateBandwidth()
	}
	/**********/
	//if 1 == frame.PathID {
	//	println("lastRcvdPacketNumber:", pth.lastRcvdPacketNumber)
	//}
	// revision start
	if err == nil {
		//s.metaLock.RLock()
		for i := 0; i < len(frame.AckRanges); i++ {
			for pktID := frame.AckRanges[i].First; pktID <= frame.AckRanges[i].Last; pktID++ {
				//delete(s.sendTime, pktID)
				//s.sizeLock.Lock()
				////s.metaLock.RLock()
				//s.sentSize += pktSize
				//println("sentSize:", s.sentSize)
				////s.metaLock.RUnlock()
				//s.sizeLock.Unlock()
				/***** added by Jinlong 7.19 *****/
				pathID := s.scheduler.packetPaths[pktID]
				s.curQuotaLock.Lock()
				if s.scheduler.curQuotas[pathID] > 0 {
					s.scheduler.curQuotas[pathID]--
				}
				s.curQuotaLock.Unlock()
				//println("Quota--:", s.scheduler.curQuotas[pth.pathID], "pathID:", pth.pathID)
				delete(s.scheduler.packetPaths, pktID)
				/**********/
				//s.metaLock.Lock()
				//delete(s.pktLen, pktID)
				//s.metaLock.Unlock()
			}
		}
		...
		if pth.rttStats.SmoothedRTT() > s.rttStats.SmoothedRTT() {
...
func (s *session) sendPackedPacket(packet *packedPacket, pth *path) error {
	...
	if isStreamFrame == true {
		if s.startTime == 0 {
			s.startTime = time.Now().UnixNano()
		}

		//s.sizeLock.Lock()
		s.sentSize += len(packet.raw)
		//println("sentSize:", s.sentSize)
		//s.sizeLock.Unlock()
		//s.metaLock.Lock()
		//s.pktLen[packet.number] = len(packet.raw)
		//s.sendTime[packet.number] = time.Now().UnixNano() / int64(time.Millisecond)
		//s.metaLock.Unlock()
	}
	// revision end
...
func (s *session) logPacket(packet *packedPacket, pathID protocol.PathID) {
	//fmt.Printf("hello-> Sending packet 0x%x (%d bytes) for connection %x on path %x, %s", packet.number, len(packet.raw), s.connectionID, pathID, packet.encryptionLevel)
	if !utils.Debug() {
		// We don't need to allocate the slices for calling the format functions
		//println("false == utils.Debug()")
		return
	}
