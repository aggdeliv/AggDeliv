...
func main() {
	...
	dialer := qproxy.NewQuicDialer(skipCertVerify)
	...
	proxy.ConnectDial = proxy.NewConnectDialToProxyWithHandler(proxyUrl,
		qproxy.SetAuthForBasicConnectRequest(username, password))

	// set basic auth
	proxy.OnRequest().Do(qproxy.SetAuthForBasicRequest(username, password))
