...
func (pm *pathManager) setup(conn connection) {
	...
	//println("before initial path:", len(pm.oliaSenders))
	pm.sess.paths[protocol.InitialPathID].setup(pm.oliaSenders)
	//println("after initial path:", len(pm.oliaSenders))
		...
		if version == 4 {
			for _, remAddr := range pm.remoteAddrs4 {
				...
				if err != nil {
					...
					n += 1
					//pm.sess.bandwidth = append(pm.sess.bandwidth, make([]uint64, 1)...)
					//pm.sess.probability = append(pm.sess.probability, make([]float64, 1)...)
				}
			}
		} else {
