    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            for i in range(self.s_dim[0]):
                split = tflearn.conv_1d(inputs[:, i:i + 1, :], 128, 4, activation='relu')
                split_flat = tflearn.flatten(split)
                split_list.append(split_flat)
            merge_net = tflearn.merge(split_list, 'concat')

            dense_net_0 = tflearn.fully_connected(merge_net, 128, activation='relu')
            dense_net_0 = tflearn.dropout(dense_net_0, 0.9)
            # dense_net_1 = tflearn.fully_connected(dense_net_0, 32, activation='relu')
            # dense_net_1 = tflearn.dropout(dense_net_1, 0.8)
